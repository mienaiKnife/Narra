...
class QueueViewModelTest {
    ...
    fun setUp() {
        ...
        viewModel = QueueViewModel(repository, feedRepository, playbackManager)
    }
    ...
    @Test
    fun `clearQueue delegates to the repository`() = runTest {
        viewModel.clearQueue()
        advanceUntilIdle()

        verify(repository).clearQueue()
    }

    @Test
    fun `removeFromQueue delegates to the repository`() = runTest {
        val article = com.mienaiknife.narra.domain.models.Article(id = "a", title = "T", source = "S")
        viewModel.removeFromQueue(article)
        advanceUntilIdle()

        verify(repository).removeFromQueue("a")
    }
}
