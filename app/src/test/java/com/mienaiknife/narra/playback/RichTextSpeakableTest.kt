/*
 * Copyright 2025 Narra Authors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.mienaiknife.narra.playback

import com.mienaiknife.narra.domain.models.RichText
import com.mienaiknife.narra.domain.models.RichTextSpan
import org.junit.Assert.assertEquals
import org.junit.Test

class RichTextSpeakableTest {
    private val linkToFormat = " link to %s "

    @Test
    fun `simple text remains unchanged`() {
        val text = "Hello World"
        val result = RichText(text).toSpeakableText(linkToFormat)
        assertEquals(text, result.text)
        assertEquals(text.length, result.text.length)
    }

    @Test
    fun `footnotes are replaced by spaces preserving length`() {
        val text = "Text[1] more text"
        val richText = RichText(text, listOf(RichTextSpan(start = 4, end = 7, isFootnote = true)))
        val result = richText.toSpeakableText(linkToFormat)
        assertEquals("Text    more text", result.text)
        assertEquals(text.length, result.text.length)
    }

    @Test
    fun `long URL links are shortened and padded to preserve length`() {
        // URL length: 43 chars
        // Simplified: " link to example.com " -> 21 chars
        // Result should have 22 spaces of padding
        val url = "https://example.com/very/long/path/to/page"
        val text = "Check out $url for more"
        val richText = RichText(text, listOf(RichTextSpan(start = 10, end = 10 + url.length, link = url)))

        val result = richText.toSpeakableText(linkToFormat)

        assertEquals(text.length, result.text.length)
        val expectedPart = " link to example.com                      "
        assertEquals(expectedPart, result.text.substring(10, 10 + url.length))
    }

    @Test
    fun `short URL links are truncated if summary is longer`() {
        // URL length: 12 chars
        // Simplified: " link to t.co " -> 14 chars
        // Result should truncate simplified to 12 chars
        val url = "https://t.co"
        val text = "See $url"
        val richText = RichText(text, listOf(RichTextSpan(start = 4, end = 4 + url.length, link = url)))

        val result = richText.toSpeakableText(linkToFormat)

        assertEquals(text.length, result.text.length)
        // " link to t.co " truncated to 12 chars is " link to t.c"
        assertEquals(" link to t.c", result.text.substring(4))
    }

    @Test
    fun `non-URL links remain unchanged`() {
        val linkText = "Click here"
        val text = "Please $linkText"
        val richText =
            RichText(
                text,
                listOf(RichTextSpan(start = 7, end = 7 + linkText.length, link = "https://example.com")),
            )

        val result = richText.toSpeakableText(linkToFormat)
        assertEquals(text, result.text)
        assertEquals(text.length, result.text.length)
    }

    @Test
    fun `multiple annotations are handled correctly`() {
        val url = "https://example.com/long"
        val text = "Text[1] and $url"
        val richText =
            RichText(
                text,
                listOf(
                    RichTextSpan(start = 4, end = 7, isFootnote = true),
                    RichTextSpan(start = 12, end = 12 + url.length, link = url),
                ),
            )

        val result = richText.toSpeakableText(linkToFormat)

        assertEquals(text.length, result.text.length)
        assertEquals("Text   ", result.text.substring(0, 7))
        // " link to example.com " (21 chars) for url (24 chars) -> 3 spaces padding
        assertEquals(" link to example.com    ", result.text.substring(12))
    }
}
