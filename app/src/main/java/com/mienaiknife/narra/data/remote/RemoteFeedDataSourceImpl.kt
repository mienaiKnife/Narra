    ...
    override suspend fun fetchFeedMetadata(url: String): Result<FeedEntity> {
        ...
        return try {
            ...
            if (channel == null) {
                ...
                val feedLink =
                    doc
                        .select("link[rel=alternate][type=application/rss+xml]")
                        .attr("abs:href")
            ...
            if (UrlUtils.isUrlOrDomainLike(title) && title.contains("://")) {
        ...
        return try {
            ...
            val articles =
                channel.items.mapNotNull { item ->
                    ...
                    val imageUrl =
                        item.image
                            ?: extractImageFromHtml(item.description).also {
                                if (it != null) android.util.Log.d("RemoteFeedDataSource", "Extracted from description: $it")
                            }
                            ?: extractImageFromHtml(item.content).also {
                                if (it !=
                                    null
                                ) {
                                    android.util.Log.d("RemoteFeedDataSource", "Extracted from content: $it")
                                }
                            }
