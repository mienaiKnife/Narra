...
- **`PlaybackService`**: A `MediaLibraryService` (a `MediaSessionService`) that hosts the `TtsPlayer`. It provides the `MediaSession` that enables system-wide media controls, lock screen integration, and foreground service persistence.
